...
def read_object(FileName, folder):
    if folder == "Input":
        file_path = BASEDIR + "/Data/%s/%s" % (folder, FileName)
    else:
        file_path = BASEDIR + "/Data/%s/%s_ModelSol" % (folder, FileName)
    
    with open(file_path, 'rb') as input_file:
        res = Pick.load(input_file, encoding="latin1")
    return res
...
def Export_Data_to_Gdx(FileName, Data):
    ...
    ws = GamsWorkspace(working_directory=BASEDIR + "/Data/Input")
    db = ws.add_database()
    ...
    Earliness_allos = db.add_parameter("Early", 0, "Maximum earliness allowed")
    Earliness_allos.add_record().value = Earliness
    Lateness_allos = db.add_parameter("Late", 0, "Maximum lateness allowed")
