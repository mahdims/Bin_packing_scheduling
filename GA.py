from utils.utils import roulette_wheel_pop
from utils.Chromo import Chromo, inbalance_measure,ItemSet
import random
import time
import math
import numpy as np
...
class GA:
    ...
    def initialpop(self):
        ...
        Bmin = int(self.Data.MinBinNo + math.ceil(2.5*self.Data.MinBinNo))
    ...
    def run(self):
        ...
        while self.iterationNO <= self.Maxit and noimprove < self.Max_noimprove and time.time() <= start+self.MaxRunTime:
            ...
            print(f"\033[96mIteration {self.iterationNO}\033[0m")
            print(" Total searched solutions: %s" % len(self.Listofsolutions))
            print(f" Elapsed time: {round(time.time() - start,2)} Best cost: {round(current_bestsol.total_cost,3)}")
