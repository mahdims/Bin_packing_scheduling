...
$GDXIN
parameter STI(ii);
STI(ii)=sum(ii2$(ord(ii2) < ord(ii)),M(ii));
N=160;
set i /1*160/;
*$gdxin %gdxincname%
...
c1(ii)..sum(i$( ord(i)>=STI(ii) and ord(i)<=STI(ii)+M(ii) ) , sum(j$(al(j,i)),alpha(j,i))) =e= y(ii);
*c2(al(j,i))..o(j,i) =l= M(i)*alpha(j,i)   ;
*c2_1(al(j,i))..alpha(j,i) =l=o(j,i)   ;

c2_2(al(j,i))$(w(i) ne w(j))..alpha(j,i)=e= 0;
c2_3(j)..sum(i$(al(j,i)), alpha(j,i)*h(i) )=l= BinH;

C3(j)$(ord(j)<=N-1)..sum(i$(al(j,i) and ord(i) ne ord(j)),alpha(j,i)) =l= (N-ord(j))*alpha(j,j) ;

c4(j)..sum(k, beta(k,j)) =e= alpha(j,j);

c5(k,j)$(ord(j)<ord(k) and ord(k)>=1 ).. sum(i$al(j,i), h(i)*alpha(j,i) ) =l= sum(i$al(k,i), h(i)*alpha(k,i)) +(BinH+1)*(1-beta(k,j)) ;
c6(k,j)$(ord(j)>=ord(k) and ord(k)<N-1 ).. sum(i$al(j,i), h(i)*alpha(j,i) ) =l= sum(i$al(k,i), h(i)*alpha(k,i)) +(BinH)*(1-beta(k,j)) ;
...
c9_2(l)$(ord(l)<=N-1) ..  sum(k$(ord(k)>=ord(l)), sum(i$al(k,i),  h(i)*alpha(k,i)) * gamma(l,k)  ) =l= BinH*gamma(l,l)   ;
...
c16(i,l).. sum((k,j)$Aset(l,k,j,i), a(l,k,j,i)*q(ii)) =l= y(ii)*BQ(l);
