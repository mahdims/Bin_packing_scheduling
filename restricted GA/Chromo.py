...
def One_Bin_Balance(Data,IB_threshold,Item_in_Bin,Revolta):
    ...
    if Item_in_Bin.inbalance >= IB_threshold:
        ...
        # explore more when its possible 
...
class level:
    ...
    def add(self,item):
        ...
        if assign==0 and item.w<=self.remain_width and self.h>=item.h:
            self.stacks.append(stack(self.h,item)) # why self.h (level h) is given to hight of stack
...
class Chromo:
    ...
    def Fitness_Calc(self, Data,pop,iterationNO,Maxit) :
        ...
        Printing_cost=sum([60*(1 + (1-b.Revolta)*b.Two_sided_item )+b.quantity*papercost for b in self.Bins])
        ...
        if iterationNO == 1:    
            ...
            self.Fitness_Value=weight*(Printing_cost/Data.Min_printing_cost)+ (1-weight)*100*(float(LT)/(Data.T))
    ...
    def Evaluation(self,Data,iterationNO,Maxit):
        ...
        # just use it every h iterations 
        h=3
        
        if  np.average([b.utility for b in self.Bins])<=0.9 and float(iterationNO/float(h)).is_integer() :
